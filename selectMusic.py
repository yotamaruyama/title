...
class SelectMusic:
    ...
    def __init__(self, screen, gameMain):
        ...
        for j in range(self.music.__len__()):
            self.music_Choise.append(self.menu_font.render(
                str(j+1)+"."+self.music[j], True, (255, 255, 255)))
    ...
    def gameninit(self):
        ...
        for i in range(self.music_Choise.__len__()):
            self.screen.blit(self.music_Choise[i], ((
                320 - (self.music_Choise[i].get_width() / 2)), 225+self.blank*i))
        # self.screen.blit(self.start, ((320 - (self.start.get_width() / 2)), 225))
        self.screen.blit(self.exit, ((320 - (self.exit.get_width() / 2)),
                         225+self.blank*self.music_Choise.__len__()))
